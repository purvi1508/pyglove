...
from pyglove.core.hyper.dynamic_evaluation import (
    dynamic_evaluate as pg_dynamic_evaluate,
)
...
class DynamicEvaluationTest(unittest.TestCase):
    ...
    def test_dynamic_evaluate(self):
        ...
        with self.assertRaisesRegex(ValueError, "'exit_fn' must be a callable object"):
    ...
    def test_evaluated_values_during_collect(self):
        with DynamicEvaluationContext().collect():
            ...
            self.assertEqual(oneof([{"x": oneof(["a", "b"])}, 1]), {"x": "a"})
            self.assertEqual(manyof(2, [0, 1, 3]), [0, 1])
            self.assertEqual(manyof(4, [0, 1, 3], distinct=False), [0, 0, 0, 0])
    ...
    def test_concurrent_asyncio_tasks_collect_and_apply(self):
        # Regression test: `DynamicEvaluationContext.apply` used to stash the
        # current decision getter on `self._decision_getter`, a plain instance
        # attribute. Concurrent `asyncio.Task`s calling `apply()` on the *same*
        # context instance (e.g. re-entrant use from concurrent LLM agent calls)
        # would race on that attribute and clobber each other's decisions, even
        # though each task never yields control to another OS thread. Decision
        # state must be isolated per asyncio.Task as well as per thread.
        context = DynamicEvaluationContext()
        with context.collect():
            oneof(range(10))

        async def task_fun(decision, expected):
            with context.apply([decision]):
                # Yield control so other concurrently-scheduled tasks run their own
                # `apply()` before this task reads back its decision.
                await asyncio.sleep(0)
                self.assertEqual(oneof(range(10)), expected)

        async def main():
            await asyncio.gather(*[task_fun(i, i) for i in range(10)])
    ...
    def test_search_space_defined_without_hyper_name(self):
        ...
        with context.apply(geno.DNA.parse([1, [0, 2], 1.5, 0, 1.8])):
            # 2 + 1 + 2 + 6 + 1.5
    ...
    def test_hierarchical_search_space(self):
        ...
        with self.assertRaisesRegex(ValueError, "No decision is provided for .*"):
            with context.apply(geno.DNA.parse(3)):
                fun()

        with self.assertRaisesRegex(ValueError, "Expect float-type decision for .*"):
            with context.apply([2, 0, 1]):
                fun()

        with self.assertRaisesRegex(ValueError, "Expect int-type decision in range .*"):
    ...
    def test_where_statement(self):
        context = DynamicEvaluationContext(where=lambda x: getattr(x, "name") != "x")
        with context.collect():
    ...
    def test_trace(self):
        ...
        self.assertEqual(
            ...
            {"decision_0": oneof([-1, 0, 1]), "decision_1": oneof([-1, 0, 3])},
        )
    ...
    def test_dynamic_evaluation_with_custom_hyper(self):
        ...
        def fun():
            return sum(IntList()) + oneof([0, 1]) + floatv(-1.0, 1.0)
        ...
        self.assertEqual(
            context.hyper_dict,
            {
                ...
                "decision_2": floatv(-1.0, 1.0),
            },
        )
        with context.apply(geno.DNA(["1:2:3:4", 1, 0.5])):
            self.assertEqual(fun(), 1 + 2 + 3 + 4 + 1 + 0.5)

        with self.assertRaisesRegex(ValueError, "Expect string-type decision for .*"):
        ...
        with self.assertRaisesRegex(
            NotImplementedError,
            ".* must implement method `next_dna` to be used in dynamic evaluation mode",
        ):
            with context.collect():
    ...
    def test_dynamic_evaluation_with_external_dna_spec(self):
        ...
        with self.assertRaisesRegex(
            ValueError, "`collect` cannot be called .* is using an external DNASpec"
        ):
    ...
    def test_search_space_partitioning_without_hyper_name(self):
        ...
        self.assertEqual(
            context1.hyper_dict, {"decision_0": oneof([1, 2, 3], hints="ssd1")}
        )
        self.assertEqual(
            context2.hyper_dict, {"decision_0": oneof([4, 5], hints="ssd2")}
        )
    ...
    def test_search_space_partitioning_with_hyper_name(self):
        ...
        self.assertEqual(
            context1.hyper_dict, {"x": oneof([1, 2, 3], name="x", hints="ssd1")}
        )
        self.assertEqual(
            context2.hyper_dict, {"y": oneof([4, 5], name="y", hints="ssd2")}
        )
        with context1.apply(geno.DNA(2)):
            with context2.apply(geno.DNA(1)):
                self.assertEqual(fun(), 3 + 5)
    ...
    def test_search_space_partitioning_with_different_per_thread_settings(self):
        ...
        with self.assertRaisesRegex(
            ValueError, "Nested dynamic evaluation contexts must be either .*"
        ):
            with context1.collect():
                with context2.collect():
                    fun()
    ...
    def test_manual_decision_point_registration(self):
        context = DynamicEvaluationContext()
        self.assertEqual(context.add_decision_point(oneof([1, 2, 3])), 1)
        self.assertEqual(context.add_decision_point(oneof(["a", "b"], name="x")), "a")
        self.assertEqual(context.add_decision_point(pg_template(1)), 1)
